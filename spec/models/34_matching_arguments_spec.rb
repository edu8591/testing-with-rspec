RSpec.describe 'matching arguments' do
  it 'can return differnet values depending on the argument' do
    ...
    expect(three_element_array.first).to eql(1)
    expect(three_element_array.first(1)).to eql([1])
    expect(three_element_array.first(2)).to eql([1, 2])
    expect(three_element_array.first(3)).to eql([1, 2, 3])
    expect(three_element_array.first(100)).to eql([1, 2, 3])
  end
end
